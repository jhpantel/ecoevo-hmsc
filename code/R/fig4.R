...
for(v.d in c(1,4,8)){ # dlev
  for(v.h2 in c(2,4,10)){ # hlev
    ## Read in population size values
    result <- paste("/Users/jhpantel/Nextcloud/Pantel/data/",h_lev[v.h2],d_lev[v.d],"_hmsc_v02.RData",sep="")
    load(result)
    s <- ncol(m.1$Y)
