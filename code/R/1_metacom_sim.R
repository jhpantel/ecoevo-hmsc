...
P = array(1,c(1,spec))    # Phenotypic variance for all species
w = array(2,c(1,spec))   # Width of fitness curve
Wmax = 2    # Maximum population growth rate for all species
Wt = array(numeric(),c(patch,spec,time))   # Record of fitness values over time, patch x spec x time
...
connec = array(0,c(spec,patch,patch))  # Species x patch x patch matrix of connectivity values
...
start = array(numeric())
for (i in 1:patch){
  if(length(n[[i]]!=0)){  # Testing if there are species in this location
    N[i,init[[i]],1] = n[[i]] # Seed initial species occupancy at half the carrying capacity
    start.1 = matrix(c(rep(i,length(init[[i]])),init[[i]]),c(length(init[[i]]),2)) #saving initial species and the patch in which they start
...
d0.1 = sqrt(B0*(w+P))
x0.1 = min(E)-d0.1     # Depends on E value for site with lowest E 
...
# Leslie-Gower growth model. Diagonal is alpha_ii, set to 0.00125. Competition
...
for(v.d in 1:length(d_vals)){
  for(v.h2 in 1:length(h2_vals)){
    ...
    for (t in 2:time) {
      ...
      if (nrow(extinct)>0) {
        for (X in 1:nrow(extinct)) {
          dt[extinct[X,1],extinct[X,2],t] =NA 
        }
      }
      ...
      comp = rowSums(pop[idx_XY[,1],]*alpha[idx_XY[,2],])      # Get the pop change due to intra- and interspecific competition   
      denom = pop
      denom[idx==0] = 1 + comp
      ...
      for (i in 1:spec) {   # for each species
        ...
        rmat =array(0,dim=c(50,max(disp_count))) 
        ...
        mig_list = lapply(rlist, check) # Checking in the list if the runif number is smaller than the dispersal likelihood (check top of script for function)
        ...
        if (sum(connec[i,,]) == 50) {disp_site=NULL} 
    ...
    save.image(paste("./data/mc/",result,"_res.RData",sep=""))
    #### Remove results
    rm(list=setdiff(ls(), c("d_vals","h2_vals","d_names","h2_names","v.d","v.h2")))
  }
}
