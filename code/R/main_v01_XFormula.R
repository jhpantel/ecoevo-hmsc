...
load(paste("././output/",result,"_res.RData",sep="")) # Scenario 1
...
if (length(c) > 0) { # if there are any extinct species
  ...
  # Remove rare species
  Y <- Y[,-e]
  # colnames(X)[c(e,e+16,e+16+15)]
  X <- X[,-c(e,e+16,e+16+15)]
}
...
sp <- s[-e]
XFormula = ~y1+y2+y4+y7+y8+y9+y10+y11+y13+y14+y15+env1+x1+x2+x4+x7+x8+x9+x10+x11+x13+x14+x15+xy1+xy2+xy4+xy7+xy8+xy9+xy10+xy11+xy13+xy14+xy14
