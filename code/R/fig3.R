...
# Date. 29.07.2025
# Description. Code for Figure 3, Pantel & Hermann 2025.
...
for(i in 1:length(d_lev)){
  for(j in 1:length(h_lev)){
    ...
    result <- paste("./data/mc/",h_lev[j],d_lev[i],"_res.RData",sep="")
    tmp.env <- new.env()
...
for(v.d in c(1)){ # dlev
  for(v.h2 in c(2,4,10)){ # hlev
    ## Read in population size values
    result <- paste("./data/mc/",h_lev[v.h2],d_lev[v.d],"_hmsc.RData",sep="")
    load(result)
    s <- ncol(m.1$Y)
