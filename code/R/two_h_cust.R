...
two_h_cust <- function (hM, Gradient, predY, measure, xlabel = NULL, ylabel = NULL, index = 1, q = c(0.025, 0.5, 0.975), cicol = rgb(0, 0, 1, alpha = 0.5), pointcol = "lightgrey", pointsize = 1, showData = FALSE, jigger = 0, yshow = NA, showPosteriorSupport = TRUE, auto=1,l1=1,h1=1,main,...) 
{
  ...
  if (is.factor(xx)) {
    ...
    if (inherits(hM$X, "list") && !measure == "Y") {
      ...
      ## JP modified
      ## Gather all data to potentially plot ##
      obj <- cbind(xx,qpred[2,],qpred[1,],qpred[3,],pX,pY,rep(1:10,times=39))
      obj <- as.data.frame(obj)
      colnames(obj) <- c("xx","qpred_50","qpred_275","qpred_975","pX","pY","val")
      obj$val <- as.factor(obj$val)
      ## Plot all elements
      ## Blank plot
      plot(qpred_50 ~ xx, data=obj,
           ylim = c(lo1, hi1),
           type = "n",
           xlab = xlabel,
           ylab = ylabel)
      ## Blue 95% ribbons
      for(i in 1:10){
        polygon(c(obj$xx[obj$val==i],rev(obj$xx[obj$val==i])),c(obj$qpred_275[obj$val==i],rev(obj$qpred_975[obj$val==i])),
                col=alpha(cicol,0.2),
                border=FALSE)
      }
      ## Grey observed points
      points(pY ~ pX, data=obj,
             pch=1,
             col="darkgrey",
             cex=0.5)
      ## Black 50% lines
      for(i in 1:10){
        lines(obj$qpred_50[obj$val==i] ~ obj$xx[obj$val==1],
              col="black",
              lwd=0.4)
      }
      ## End Modified JP ##
