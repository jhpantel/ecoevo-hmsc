...
df <- data.frame(cbind(log(dat$N1[2:t]),log(dat$N2[2:t]),log(dat$N1[1:(t-1)]),log(dat$N2[1:(t-1)])))
...
```
With the reduced impact of the environment, we now see the strong negative associations between the two competing species. We also correctly see a relatively weak impact of environment for species abundances.
...
studyDesign = data.frame(sample = as.factor(1:(t-1)))
rL = HmscRandomLevel(units = studyDesign$sample)
m.7.hmsc = Hmsc(Y = Y, XData = XData, XFormula = ~E+Esq+dx1+dx2, studyDesign = studyDesign, ranLevels = list(sample = rL.sample))
# Bayesian model parameters
nChains <- 2
thin <- 5
samples <- 2000
...
studyDesign = data.frame(sample = as.factor(1:(t-1)))
rL = HmscRandomLevel(units = studyDesign$sample)
m.8.hmsc = Hmsc(Y = Y, XData = XData, XFormula = ~E+Esq+dx1+dx2, studyDesign = studyDesign, ranLevels = list(sample = rL.sample))
# Bayesian model parameters
nChains <- 2
thin <- 5
samples <- 2000
...
# Species growth, competition, environmental change, and trait evolution in a spatially structured environment

We now consider that trait evolution can occur, with an impact on the species growth rate / fitness in the context of the environment.

A final consideration to add to the model is the existence of multiple sites that may have correlation structure in the variance of species abundances. We consider this in our simulation by adding multiple (10) sites, with no vs. high spatial covariance. We then consider spatial random effects in HMSC, where covariance is considered as a function of distance between sites.
...
t <- 40
N <- array(NA,dim=c(j*t,2))
N <- as.data.frame(N)
colnames(N) <- c("N1","N2")
x <- array(NA,dim=c(j*t,2))
x <- as.data.frame(x)
colnames(x) <- c("x1","x2")
r <- array(NA,dim=c(j*t,2))
r <- as.data.frame(r)
colnames(r) <- c("r1","r2")
N$N1[1:j] <- N1.0
N$N2[1:j] <- N2.0
...
x$x2[1:j] <- x2.0
What <- Wmax*sqrt(w/(P+w))
r$r1[1:j] <- What*(exp((-(((w+(1-h2)*P)/(P+w))*(E.0-x1.0))^2)/(2*(P+w))))
r$r2[1:j] <- What*(exp((-(((w+(1-h2)*P)/(P+w))*(E.0-x2.0))^2)/(2*(P+w))))
E <- rep(NA, t)
E[1] <- E.0
...
for (i in 2:t) {
  for(z in 1:j){
    res <- disc_LV_evol(N1.0=N[study$site==z & study$time==i-1,1],N2.0=N[study$site==z & study$time==i-1,2],alpha.11=alpha.11,alpha.22=alpha.22,alpha.12=alpha.12,alpha.21=alpha.21,E=E[i-1],x1=x[study$site==z & study$time==i-1,1],x2=x[study$site==z & study$time==i-1,2],P=P,w=w,Wmax=Wmax,h2=h2)
    N$N1[study$site==z & study$time==i] <- res[1]
    N$N2[study$site==z & study$time==i] <- res[2]
    r$r1[study$site==z & study$time==i] <- res[3]
    r$r2[study$site==z & study$time==i] <- res[4]
    # trait change
    d1 <- E[i-1] - x[study$site==z & study$time==i-1,1]
    d2 <- E[i-1] - x[study$site==z & study$time==i-1,2]
    d1.1 <- k*d1
    d2.1 <- k*d2
    x$x1[study$site==z & study$time==i] <- E[i-1] - d1.1
    x$x2[study$site==z & study$time==i] <- E[i-1] - d2.1
  }
  # environmental change
  E[i] <- E[i-1] + abs(rnorm(1, 0, 0.05))
}
...
ggplot2::ggplot(dat, aes(time, value, col=variable)) + geom_point() + facet_wrap(~site,nrow=2)
```
...
```
We can see that the model considers spatial random effects by removing the spatial covariance, and only including spatial variance.
