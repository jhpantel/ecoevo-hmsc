...
rL.sample = HmscRandomLevel(units = studyDesign$sample)
m.7.hmsc = Hmsc(Y = Y, XData = XData, XFormula = ~E+Esq+dx1+dx2, studyDesign = studyDesign, ranLevels = list(sample = rL.sample,time=rL.time))
# Bayesian model parameters
nChains <- 2
thin <- 5
samples <- 2000
...
plotVariancePartitioning(m.7.sample,VP,cols=c("white","skyblue","darkgrey","orange","black"),args.legend=list(cex=0.75,bg="transparent"))
```
...
m.8.hmsc = Hmsc(Y = Y, XData = XData, XFormula = ~E+Esq+dx1+dx2, studyDesign = studyDesign, ranLevels = list(sample = rL.sample,time=rL.time))
# Bayesian model parameters
nChains <- 2
thin <- 5
samples <- 2000
...
plotVariancePartitioning(m.8.sample,VP,cols=c("white","skyblue","darkgrey","orange","black"),args.legend=list(cex=0.75,bg="transparent"))
```
...
studyDesign = data.frame(sample = as.factor(1:(j*(t-1))),time=rep(1:(t-1),each=j),site=rep(1:j,times=(t-1)))
studyDesign$time <- as.factor(studyDesign$time)
studyDesign$site <- as.factor(studyDesign$site)
rL.time = HmscRandomLevel(units = studyDesign$time)
rL.spatial = HmscRandomLevel(sData = xycoords)
# Fit HMSC model
m.9.hmsc = Hmsc(Y = Y, XData = XData, XFormula = ~E+Esq+dx1+dx2, studyDesign = studyDesign, ranLevels = list("time" = rL.time,"site"=rL.spatial))
# Bayesian model parameters
nChains <- 2
thin <- 5
samples <- 2000
...
plotVariancePartitioning(m.9.sample,VP,cols=c("white","skyblue","darkgrey","orange","black"),args.legend=list(cex=0.75,bg="transparent"))
```
