...
**Statistical model**
The goal of fitting the observed data to the HMSC model was to determine whether HMSC could successfully estimate the overall effects of four main drivers of population size for each of the species in the simulated metacommunity: environment, population size at the previous time step, evolution, thus change in species' trait $x$, and the interaction between population size and change in trait $x$. The full simulated time series was used as data to fit to the HMSC model, and therefore (1) the predictive power of the model was high ($R^2$ > 0.98 for all species, in all four model scenarios) and (2) using the population size at the previous time step was sufficient to explain almost of all the variation in the HMSC model (Appendix XX VarPart results). Instead of comparing the environmental, density, and evolutionary predictors on their importance for overall population size and explanatory power, we instead look at the values of the estimated effect size for scaled (to mean zero and unit variance) predictors for a single set of representative simulation runs, when dispersal $d = 10^{-3}$ and heritability $h^2 = 0.1$ for all species. 

There was no observed strong, consistent effect of changing the strength of selection (weak selection: $P = 1, w = 10$, strong selection: $P = 0.1, w = 0.2$) or the degree of environmental variability (no environmental variability: $v[E] = 0$, environmental variability: $v[E] = 0.01$) for the overall Intercept, the effects of environment, the intraspecific predictors (Figure \@ref(fig:fig4)), or the interspecific impacts of species densities on densities of other species (Figure \@ref(fig:fig5)). However, these differing simulation scenarios did impact the interspecific trait evolution predictors: scenarios with weak selection had a much stronger impact of $|\Delta x_{ik,t-1 \to t}|$ for abundances of other species, and scenarios without environmental variability had a much stronger impact of the interaction term for population size by change in trait value $N_{ik,t-1} \times |\Delta x_{ik,t-1 \to t}|$ (Figure \@ref(fig:fig5)).

It is interesting to look in more depth at the impacts that trait evolution - quantified as the absolute value of the change in the population's trait value $x$ from one time step to the next ($|\Delta x_{ik,t-1 \to t}|$), have for the population sizes of other species ($y_i$). In Scenario 1 (weak selection, no environmental variability), we observe that $|\Delta x_{15}|$ has a very strong impact for $y_{13}$, with a posterior mean of `r mean(m.df$Bx13.15)` (and 95% HPDI, or 95% highest probability density interval = `r quantile(m.df$Bx13.15,probs=c(0.025,0.975))`). Figure 6 \@ref(fig:fig6) shows that high population densities of species 13 correlate with high trait changes of species 15 in all 5 patches, where both species co-exist. These indicate that species 15 had its greatest trait changes at the start of the simulation during which species 13 reached high population densities. As species 15 declines due to the increase of species 2, species 13 recovers again its population size (Fig. 6b,h,j). Similarly we also observe a strong impact of $|\Delta x_{2}|$ for $y_{15}$, with a posterior mean of `r mean(m.df$Bx15.2)` (and 95% HPDI, or 95% highest probability density interval = `r quantile(m.df$Bx15.2,probs=c(0.025,0.975))`). It is important to note that interspecific impacts of $|\Delta x_{i}|$ are always indirect, as there is no direct interaction between $x_i$ and $y_j$. The only possible form of influence of is an increase in $y_i$ through evolution, leading to changes in intraspecific competition. We studied these posteriors in more detail, to provide an example on how to interpret these parameter estimations by the HMSC model (see Appendix XX). 
...
--->
When interpreting estimated posteriors of the HMSC model, for the different parameters, it is important to consider that these are based on correlations. Such a correlation may be the result of a direct impact from one parameter, the effect of an unmeasured parameter or the result of different parameter interacting. Therefore, when interpreting their effects on the communitiy processes they need to be investigated further. As an example for this we examined the higher posterior of $|\Delta x_{15}|$ on $y_{13}$ and of $|\Delta x_{2}|$ on $y_{15}$ (for further detail see Appendix XX). In this example we were able to show that the effect of $|\Delta x_{2}|$ on $y_{15}$ was a direct consequence of the strong competition coefficient between both species and as species 2 was the stronger competitor (through higher population size) it led to the decline of $y_{15}$. At the same time the effect of $|\Delta x_{15}|$ on $y_{13}$ was the result of a correlation without causation, as species 13 has no strong competition with species 15 (or 2). Therefore, before concluding effects of the estimated posteriors these parameters need to be further analyzed. Furthermore, results from species with low densities need to be interpreted with care as the low population size might inflate the resulting posterior.
...
p_Y<-list()
for (i in 1:length(b)) {
patches[[i]] <- data.frame(x15=X3d[b[i],23,],y02=Y3d[b[i],2,],y13=Y3d[b[i],9,],y15=Y3d[b[i],11,],time=2:1000)  
...
p_X[[i]] <-ggplot(patches_l[[i]],aes(x=x15,y=Y,color=sp,linetype=sp))+
  ...
  ylim(c(0,700))+
  scale_color_manual(values = c("coral3","darkorchid4","rosybrown1"))+
  theme_bw()+
  xlab(NULL)+ylab(NULL)+
  theme()+
...
```{r fig8, echo=FALSE, fig.show="hold", out.width="60%", fig.cap = "Antagonistic (HPDI < 0), additive (HPDI includes 0), or synergistic (HPDI > 0) eco-evolutionary interaction effects for (a) scenario 1 (weak selection, no environmental variability), (b) scenario 2 (strong selection, no environmental variability), (c) scenario 3 (weak selection, environmental variability), and (d) scenario 4 (strong selection, environmental variability)."}
# Load data
load("../output/h_01_d_minus3_hmsc_v01.RData")
# Settings for number of species and their identity
s <- ncol(m.1$Y)
