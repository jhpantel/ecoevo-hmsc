...
ggplot(data=dat,aes(x=time)) +
  ...
  geom_line(aes(y=ar1.fit, color = "lm fit"), alpha=0.5, linewidth=.4) +
  theme_bw() +
  xlab("time") +
  ylab("N") +
  labs(color="Legend") +
...
```{r figs5, fig.pos = "H",echo=FALSE, fig.show="hold", fig.cap = "\\label{fig:figs5} Observed (grey) and model-fit (blue) values for population size at time t (x-axis) and t+1 (y-axis).",results='hide',fig.keep='all'}
...
Here, $\hat{W}$ is the maximal population growth rate (set to `r r1.0` as above), $E$ is the local environmental trait optimum value, and $x_{i,t}$ is species *i* trait value at time *t*. We simulate the environment $E$ value fluctuating randomly over time, and use a linear model to fit $E$ as a covariate:
...
E <- 0.8
x1.0 <- 0.7
x_range <- seq(0,2,by=0.01)
r_range <- r1.0*exp(-(E-x_range)^2)
plot(x_range,r_range)
```
